...
void start_johnny_worker() {
    johnny_worker(johnny_setup_worker());
}

void set_thread_cpu(pthread_t thread_id, int core_id) {
    cpu_set_t cpuset;
    CPU_ZERO(&cpuset);
...
void create_johnny_worker_thread(int core_id) {
    pthread_t thread_id;
...
void start_a_johnny_worker_on_each_core() {
